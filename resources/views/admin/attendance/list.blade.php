...
<td>
    @php
        $actual = \Carbon\Carbon::parse($attendance->datetime); // actual check-in

        if (!empty($attendance->date) && !empty($attendance->time)) {
            // Both date & time given
            $expected = \Carbon\Carbon::parse($attendance->date . ' ' . $attendance->time);
        } elseif (!empty($attendance->time)) {
            // Only time given → same date as actual
            $expected = \Carbon\Carbon::parse($actual->format('Y-m-d') . ' ' . $attendance->time);
        } else {
            $expected = null;
        }
    ...
    @elseif($diffMinutes > -30 && $diffMinutes <= 30)
...
function filterAttendance() {
    ...
    // Show print button only if filtered results exist
